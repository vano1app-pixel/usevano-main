...
type GoogleSignInButtonProps = {
  ...
  loading?: boolean;
  className?: string;
  /** Slightly smaller copy for secondary placement */
...
export function GoogleSignInButton({
  ...
  return (
    <button
      ...
      className={cn(
        'w-full inline-flex items-center justify-center gap-3 rounded-xl font-semibold transition-colors',
        'border border-border bg-background text-foreground shadow-sm',
        'hover:bg-muted/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring',
        'disabled:opacity-50 disabled:pointer-events-none',
        isSecondary ? 'py-2.5 text-sm' : 'py-3.5 text-[15px] min-h-[48px]',
        className,
      )}
    >
