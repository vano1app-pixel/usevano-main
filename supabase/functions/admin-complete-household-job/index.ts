...
async function sendSms(to: string | null | undefined, body: string): Promise<boolean> {
  ...
  // WhatsApp preferred — no Irish carrier filtering.
  ...
  if (waFrom) {
    ...
    try {
      ...
      if (!resp.ok) console.warn('[complete whatsapp] twilio error', resp.status, (await resp.text()).slice(0, 200));
      else console.log(`[complete whatsapp] sent to ${e164}`);
      return resp.ok;
    } catch (e) {
      console.warn('[complete whatsapp] twilio exception', e);
      return false;
    }
  }
