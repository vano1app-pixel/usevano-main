...
// helper sees BEFORE they've claimed a job.
//
// WHY (owner call 2026-07-30): "don't show the eircode till the helper
// accepts, just give them a roundabout area". Two problems this solves:
//   1. A LEAK — StudentJobDetail rendered the full customer_address on an
//      UNCLAIMED job with no ownership guard (the phone number was gated,
//      the address wasn't). Any helper opening the job link saw the exact
//      house of a customer who had not yet agreed to have them come.
//   2. USELESS PRECISION — the open-jobs board and dispatch offers only said
//      "Galway", which tells a helper nothing about whether the job is a
//      5-minute walk or a 40-minute bus. "Salthill" is the honest middle:
//      enough to decide, not enough to identify a house.
//
// THE INVARIANT: this module can only ever return a name from the fixed
// table below, or the city, or null. It NEVER echoes any part of a typed
// address back out — that's what keeps "12 Oak Drive" from leaking through
// a free-text path. Address matching is allowlist-only (does the address
// CONTAIN a known area name), never extraction.
//
// Pure TypeScript, no Deno APIs — vitest imports it directly and
// src/lib/serviceAreas.ts mirrors it for the frontend; serviceAreas.test.ts
// holds the two in lock-step (same pattern as the pricing tables).
