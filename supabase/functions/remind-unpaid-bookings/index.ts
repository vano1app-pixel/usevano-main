...
async function sendSms(to: string | null | undefined, body: string): Promise<boolean> {
  ...
  if (waFrom) {
    ...
    try {
      ...
      if (!resp.ok) console.warn('[remind-unpaid whatsapp] twilio error', resp.status, (await resp.text()).slice(0, 200));
      return resp.ok;
    } catch (e) {
      console.warn('[remind-unpaid whatsapp] twilio exception', e);
      return false;
    }
  }
